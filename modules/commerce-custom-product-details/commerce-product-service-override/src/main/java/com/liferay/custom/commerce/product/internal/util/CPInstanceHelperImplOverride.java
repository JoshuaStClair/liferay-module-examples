...
package com.liferay.custom.commerce.product.internal.util;
...
/**
 * Code is copied from 2.0.6
 * @see <a href="https://github.com/liferay/com-liferay-commerce/blob/2.0.6/commerce-product-service/src/main/java/com/liferay/commerce/product/internal/util/CPInstanceHelperImpl.java.java">CPInstanceHelperImpl.java</a>
 *
 * All this code is copied exactly from source with the exception of source formatting and one customization added to
 * the renderCPInstanceOptions() method in order to support our custom "table" option. The original component needed to
 * be blacklisted because the component policy is not greedy so a higher service ranking alone will not cause our
 * override to be picked up. Furthermore, the place it is called, CPInstanceDisplayContext, is not an OSGi component so
 * we can't redirect that reference to point to our override.
 *
 * @author Marco Leo
 * @author Alessio Antonio Rendina
 */
@Component(immediate = true, service = CPInstanceHelper.class)
public class CPInstanceHelperImplOverride implements CPInstanceHelper {
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			for (int j = 0; j < valueJSONArray.length(); j++) {
				...
				CPDefinitionOptionValueRel cpDefinitionOptionValueRel =
					_cpDefinitionOptionValueRelLocalService.fetchCPDefinitionOptionValueRel(
	...
	public List<CPDefinitionOptionValueRel> getCPDefinitionOptionValueRel(
		...
		return _cpDefinitionOptionValueRelLocalService.getCPDefinitionOptionValueRels(cpDefinitionOptionValueRelIds);
	}
	...
	public String renderCPInstanceOptions(
		...
		// Custom Start
		// This customization works by displaying the option selection as a list drop down when adding a new SKU.
		// Otherwise, a table would render which doesn't allow the user to pick which option they want to bind the new
		// SKU to.

		if (!Objects.isNull(ddmForm)) {
			for (DDMFormField ddmFormField : ddmForm.getDDMFormFields()) {
				if (ddmFormField.getProperty("type").equals("table")) {
					ddmFormField.setProperty("type", "select");
				}
			}
		}
	...
	private boolean _isDDMFormRequired(
		...
		if (Validator.isNotNull(fieldTypeDataDomain) && fieldTypeDataDomain.equals("list")) {
			int cpDefinitionOptionValueRelsCount =
				_cpDefinitionOptionValueRelLocalService.getCPDefinitionOptionValueRelsCount(
					cpDefinitionOptionRel.getCPDefinitionOptionRelId());
		...
		if (publicStore) {
			if (cpDefinitionOptionRel.isRequired() || cpDefinitionOptionRel.isSkuContributor()) {
				return true;
			}
		}
		else {
